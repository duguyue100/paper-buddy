#!/usr/bin/env python3
# ponytail: runtime style analyzer for paper-style skill.
# Measures a draft with the SAME extractor core as the corpus, compares to
# cvpr_stats.json, prints a structured report: field, your_value, corpus median,
...
STATS_PATH = os.path.join(HERE, "..", "reference", "cvpr_stats.json")
...
def main():
    ...
    for k in ("CV_conf", "ML_conf", "preprint", "journal", "other"):
    ...
    print(f"  (corpus per-paper median: CV_conf {cv_med}%, ML_conf {ml_med}%)")
