#!/usr/bin/env python3
# ponytail: aggregate 267 per-paper features into the skill reference files.
# Outputs: skills/paper-style/reference/{acl_stats.json, acl_style.md},
#          skills/paper-critic/reference/acl_expectations.md.
...
MIN_DOMAIN_N = 5  # ponytail: per-domain medians noisy below that; skip if fewer.
...
def is_readable_sentence(s):
    # ponytail: reject sentences that are mostly LaTeX junk (option keys, table cells, math).
    # Heuristic: alpha ratio > 0.55 over non-space chars; reject if it contains option-key
    # chunks like "colback=GrayBack" or starts with `[1]`-style bracket refs alone.
    non_space = [c for c in s if not c.isspace()]
    if not non_space:
        return False
    alpha = sum(1 for c in non_space if c.isalpha())
    if alpha / len(non_space) < 0.55:
        return False
    if re.search(r"\bcol\w+=\w+|back=\w+|frame=\w+|rule=\w+|=Gray|=\d+pt\b", s, re.I):
        return False
    if re.match(r"^\[\d+\]", s):
        return False
    return True
...
def grep_sentence(text, pattern, max_len=200):
    ...
    for m in re.finditer(r"[^.!?\n]{15," + str(max_len) + r"}[.!?]", blob):
        s = m.group(0).strip()
        if re.search(pattern, s, re.I) and len(s) < max_len and is_readable_sentence(s):
            return s
    return None
...
def pick_quote_hook(feats, hook):
    for f in feats:
        if f["intro"].get("hook_type") == hook:
            aid = f["id"]
            txt = read_paper_text(aid, name_contains="intro")
            # ponytail: for "question" hooks require a "?" so we quote an actual question;
            # for "claim"/"statistic" hooks the first readable sentence suffices.
            pat = r"\?" if hook == "question" else r"."
            s = grep_sentence(txt, pat, max_len=200)
            if s:
                return s, aid
    return None, None
...
def build_stats_json(feats):
    ...
    # citation venue share aggregate + per-paper medians (NLP_conf primary, ML_conf secondary)
    from collections import Counter
    agg = Counter()
    for f in feats:
    ...
    nlp_pp = [f["citations"]["venue_counts"]["NLP_conf"] / max(1, sum(f["citations"]["venue_counts"].values()))
    ...
    stats["fields"]["citations.NLP_conf_perpaper_median"] = round(S.median(nlp_pp) * 100, 1)
...
def build_style_md(feats, stats):
    L = []
    L.append("# ACL 2026 — Style & Structure Reference\n")
    L.append(f"Aggregated from **{len(feats)} accepted oral papers**. Generated by "
              "`aggregate.py` from `features/*.json`. Source papers were scraped from arXiv "
              f"by matching the ACL 2026 oral list ({len(feats)}/478 oral papers had arXiv "
              "preprints with LaTeX sources).\n")
    L.append("Use these as an ACL yardstick. Each numeric field is the corpus median and "
             "inter-quartile range; if a flagged field is *domain-sensitive*, the range varies "
             "by sub-task (LLMs vs generation vs retrieval vs parsing), so deviations are "
             "defensible with a domain-specific reason, not a violation of the convention. "
             "Quotes are verbatim excerpts (attributed by arxiv id) — do not paraphrase.\n")
    ...
    L.append(f"- ACL abstracts rarely enumerate contributions — the contribution list lives in "
             f"the intro (~{bl['rate_true']}% of orals put one there, even fewer in the abstract).\n")
    ...
    L.append(f"- **{bl['rate_true']}%** have an explicit 'Ablation' (sub)section heading — note: "
             "lower than CVPR/ICML; many ACL papers fold design-choice isolation into an "
    ...
    L.append("- **Citation venue share (aggregated, as % of classified bib entries):** "
             + ", ".join(f"{k}={v}%" for k, v in sh.items()))
    L.append(f"- **Per-paper NLP-family (ACL/EMNLP/NAACL/COLING/TACL/Findings) cite share**: "
             f"median {stats['fields']['citations.NLP_conf_perpaper_median']}% — note: NLP "
             "conferences are a *minority* of citations in modern ACL papers; authors draw "
             "heavily from arXiv preprints, ML conferences (NeurIPS/ICML/ICLR), and journals "
             f"(per-paper ML conf share median {stats['fields']['citations.ML_conf_perpaper_median']}%). "
             "Use this metric as a sanity check on community-anchoring, not a quota.\n")
    ...
    L.append("\n_Domains are a coarse keyword tag over title+abstract (ceiling: upgrade to "
             "embeddings to refine). When a metric is flagged domain-sensitive, prefer the "
             "per-domain median of the author's closest domain over the overall median._\n")
    return "\n".join(L)

def build_expectations_md(stats):
    L = ["# ACL 2026 — Critic Expectations (compact)",
         "",
         f"Numbers below are corpus medians from {stats['n_papers']} accepted orals. Use them "
         "to flag drafts that deviate *and provide no domain reason*. A deviation is not "
         "automatically wrong.",
         ""]
    f = stats["fields"]
    q = lambda k: f[k]["quantiles"]
    L.append("## Minimum structural skeleton an ACL draft should show")
    L.append("- Abstract (typically 5–10 sentences, 100–200 words)")
    L.append("- Introduction with an explicit contribution enumeration (a minority of orals do)")
    L.append(f"- Related Work (typical ~{fmt(q('related.words')['median'])} words)")
    L.append(f"- Method block — equation density varies heavily by task; many ACL methods are "
             f"prompt-based or pipeline-based with {fmt(q('method.equation_count')['median'])} "
             f"equation environments (median).")
    L.append(f"- Experiments with ≥{fmt(q('experiments.table_count')['median'])} tables and an "
             "ablation OR analysis section that isolates design choices.")
    L.append("- Conclusion")
    L.append("")
    L.append("## What the corpus flags as reviewer-style red flags")
    L.append("- Missing or stub 'Ablation'/'Analysis' subsection — ACL papers isolate design "
             "choices slightly less often than CVPR (~44% have an explicit Ablation heading), "
             "but the analysis duty does not vanish; if no section isolates the contributions, "
             "reviewers will ask. Frame the ablation around which design choice drove the gain.")
    L.append("- SOTA claims without a paired quantitative result table reference — superlative "
             f"mentions median {fmt(q('experiments.sota_mentions')['median'])} per paper; each "
             "should map to a table or figure.")
    L.append("- Citation drift: if your draft's NLP-family (ACL/EMNLP/NAACL/COLING/TACL/Findings) "
             f"cite share is far below the corpus median (~{f['citations.NLP_conf_perpaper_median']}%), "
             "verify you are anchoring in the venue's prior work. ACL authors lean heavily on "
             "arXiv and ML conferences, so a near-zero NLP-fraction in 2026 is a strong signal "
             "the paper is grounded in ML rather than NLP — fine if intentional, suspicious if "
             "submitted to ACL. Cite-share is a heuristic, not a rule.")
    L.append("")
    L.append("## Counts vary by domain (do not enforce blindly)")
    for k in ("method.equation_count", "experiments.table_count",
              "experiments.ablation_mentions", "experiments.sota_mentions", "related.words"):
...
def main():
    ...
    with open(os.path.join(STYLE_DIR, "acl_stats.json"), "w") as fh:
        json.dump(stats, fh, indent=2)
    with open(os.path.join(STYLE_DIR, "acl_style.md"), "w") as fh:
        fh.write(build_style_md(feats, stats))
    with open(os.path.join(CRITIC_DIR, "acl_expectations.md"), "w") as fh:
        fh.write(build_expectations_md(stats))
    print(f"[done] wrote acl_stats.json, acl_style.md (paper-style), "
          f"acl_expectations.md (paper-critic). n={len(feats)}.")
