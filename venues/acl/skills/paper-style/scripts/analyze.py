#!/usr/bin/env python3
# ponytail: runtime style analyzer for paper-style skill (ACL edition).
# Measures a draft with the SAME extractor core as the corpus, compares to
# acl_stats.json, prints a structured report: field, your_value, corpus median,
# P25/P75, status (normal/deviates/missing/domain-sensitive-deviation).
# Usage: python analyze.py <draft.tex or dir>
# Repo-coupled: imports extract.py two levels up; symlink-install the skills dir.
import os, sys, json
...
STATS_PATH = os.path.join(HERE, "..", "reference", "acl_stats.json")
...
def status_for(value, q, domain_sensitive=False):
    # ponytail: normal within IQR; deviates outside P75/P25; missing if None.
    if value is None:
        return "missing"
    if q is None:
        return "no_corpus_data"
...
def main():
    ...
    if os.path.isdir(target):
        ...
        # single file: wrap in a temp-like dir resolution by using its dirname
    ...
    print(f"domain (keyword tag): {feats.get('domain','?')}")
    ...
    for k in ("NLP_conf", "ML_conf", "CV_conf", "preprint", "journal", "other"):
        print(f"  {k}: {100*vc.get(k,0)/tot:.1f}%")
    nlp_med = stats["fields"].get("citations.NLP_conf_perpaper_median")
    ml_med = stats["fields"].get("citations.ML_conf_perpaper_median")
    print(f"  (corpus per-paper median: NLP_conf {nlp_med}%, ML_conf {ml_med}%)")
