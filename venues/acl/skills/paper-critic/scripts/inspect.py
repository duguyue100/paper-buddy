#!/usr/bin/env python3
# ponytail: runtime draft inspector for paper-critic skill (ACL edition).
# Extracts section text + structural critique signals from a draft so the
# model can apply critique_checklist.md deterministically (no model opining
# on the raw tex). Repo-coupled: imports extract.py two levels up.
...
ANALYSIS_RE = re.compile(r"analysis", re.I)

def clean_for_read(text, max_chars=3500):
    # ponytail: demote shaping macros so visible content survives, strip other cmds.
...
def signals_for_section(sec_text):
    ...
    # ponytail: a superlative with no \ref nearby (within 200 chars) is a red flag.
    unanchored = 0
    ...
    return {
        ...
        "analysis_mentions": len(ANALYSIS_RE.findall(sec_text)),
        "word_count": extract.count_words(sec_text),
    }

def pick_section(body, sections, spans, want):
    # ponytail: fuzzy match section title by substring.
    if not sections:
        return None, "", -1
    want = want.lower()
...
def main():
    ...
    if want:
        ...
        if title is None:
            # try abstract as named block
            print(f"## Requested section '{want}' NOT FOUND. Available sections listed above.")
            sys.exit(0)
        blocks.append((title, txt))
    else:
        # default: harvest abstract + intro + experiments + any ablation/analysis block
        ...
        # an ablation block: search section and subsection headings
        abl_idx = None
        for i, s in enumerate(sections):
            if ABLATION_RE.search(s) or ANALYSIS_RE.search(s.lower()):
                abl_idx = i; break
    ...
    for title, txt in blocks:
        ...
        print(f"  words={sig['word_count']}, superlatives={sig['superlative_mentions']} "
              ...
              f"analysis_mentions={sig['analysis_mentions']}, "
    ...
    print("\n## Structural completeness (vs ACL expectations)")
    ...
    ana = any(ANALYSIS_RE.search(s.lower()) for s in sections)
